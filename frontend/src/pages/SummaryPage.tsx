  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8 overflow-y-auto">
      <div className="text-center mb-6 sm:mb-8">
        ...
        <p className="text-white/40 mt-1 text-sm">Side-by-side overview</p>
      </div>
...
function ProviderCard({ clinic, isTop, onSelect }: { clinic: Clinic; isTop: boolean; onSelect: () => void }) {
  return (
    <div className={`glass-card p-4 sm:p-5 flex flex-col ${isTop ? 'border-teal-500/20' : ''}`}>
      <div className="mb-4">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          ...
          {clinic.badges.highVisits && <StatusBadge status="high-visits" />}
        </div>
      ...
      {clinic.patientSummary && (
        <p className="text-white/40 text-xs mt-4 leading-relaxed">{clinic.patientSummary}</p>
      )}
